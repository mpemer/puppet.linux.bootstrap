...
> " response
        ...
        then
            ...
            [ -e /root/.ssh/id_rsa ] && $SUDO mv /root/.ssh/id_rsa /root/.ssh/id_rsa.$ts 
            [ -e /root/.ssh/id_rsa.pub ] && $SUDO mv /root/.ssh/id_rsa.pub /root/.ssh/id_rsa.pub.$ts 
        else
    ...
    if $create_key
    then
        $SUDO ssh-keygen -b 2048 -t rsa -C "RSA key generated by $0" -N '' -f /root/.ssh/id_rsa
...
if which yum
then
    $SUDO wget -O /etc/yum.repos.d/public-yum-el5.repo http://public-yum.oracle.com/public-yum-el5.repo
    ...
    then
        ...
        $SUDO apt-get -q -y --force-yes install git puppet
    else
...
$SUDO GIT_SSL_NO_VERIFY=true git clone $git_url /etc/puppet

# Make a log file for puppet
puppet_log_file=/var/log/puppet/puppet.log
$SUDO touch $puppet_log_file
$SUDO chmod 600 $puppet_log_file

# Run puppet
$SUDO timeout -k 10 290 \
nice -n 19 \
puppet apply \
