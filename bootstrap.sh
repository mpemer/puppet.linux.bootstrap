...
# We want to run some commands as root
if [ "$UID" -ne 0 ]
then
    if which sudo
    then
...
> " response
    ...
    if $create_key
    then
        bash -c "$SUDO ssh-keygen -b 2048 -t rsa -C 'RSA key generated by bootstrap' -N '' -f /root/.ssh/id_rsa"
...
"
        bash -c "$SUDO cat /root/.ssh/id_rsa.pub"
...
if which yum
then
    bash -c "$SUDO wget -O /etc/yum.repos.d/public-yum-el5.repo http://public-yum.oracle.com/public-yum-el5.repo"
    bash -c "$SUDO rpm -Uvh http://dl.fedoraproject.org/pub/epel/5/i386/epel-release-5-4.noarch.rpm"
    bash -c "$SUDO yum -y install puppet git"
else    
    if which apt-get
    then
        # Update our system from bare minimum to usable level
        bash -c "$SUDO apt-get -q -y --force-yes update"
        bash -c "$SUDO apt-get -q -y --force-yes install git puppet"
    else
...
# Check out git repo
bash -c "$SUDO rm -fR /etc/puppet"
bash -c "$SUDO GIT_SSL_NO_VERIFY=true git clone $git_url /etc/puppet"

# Make a log file for puppet
puppet_log_file=/var/log/puppet/puppet.log
bash -c "$SUDO touch $puppet_log_file"
bash -c "$SUDO chmod 600 $puppet_log_file"

# Run puppet
bash -c "$SUDO timeout -k 10 290 nice -n 19 puppet apply --environment=production --modulepath /etc/puppet/modules --templatedir /etc/puppet/templates --logdest $puppet_log_file /etc/puppet/manifests/init.pp"
